...
# 원형 장애물 (중심 x, 중심 y, 반지름 r) - 자율주행에서는 값이 동적으로 들어옴.
OBSTACLES = [
...
def cast_lidar(x, y, theta, num_rays=NUM_RAYS, fov=FOV, max_range=MAX_RANGE, step=STEP):
    ...
    dists = np.full(num_rays, max_range, dtype=float) # 거리 배열 초기화: 전부 max_range :레이와 레이사이의 간격
